...
class PerformanceOptimizingUtil:
    ...
    def gen_dictionary_for_monitor_target_edited_and_bkup(directory_abspath):
        r"""
        monitor_target_edited_and_bkup() 성능개량용 코드
        트라이 구조 딕셔너리 적용
        ...
        """
        current_directory_state = BusinessLogicUtil.get_directory_files_files_for_monitoring_mtime_without_files_to_exclude(directory_abspath=directory_abspath)  # str to dict
...
class FileSystemUtil:
    ...
    def convert_mp4_to_webm(target_abspath):
        ...
        os.chdir('storage')
        os.system(f'"{BusinessLogicUtil.FFMPEG_EXE}" -i "{target_abspath}" -f webm -c:v libvpx -b:v 1M -acodec libvorbis "{file_edited}" -hide_banner')
            ...
            if not os.path.exists(future_abspath):
                ...
                if result_sync:
                    ...
                    if os.path.exists(StateManagementUtil.DIRSYNC_LOG):
                        ...
                        UiUtil.pop_up_as_complete(title="작업완료메시지", ment=f"타겟의 동기화가 완료 되었습니다\n{future_abspath}", auto_click_positive_btn_after_seconds=10)
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
        # from dirsync import sync
    ...
    def is_directory_changed(directory_abspath):
        # 현재 디렉토리 상태
        try:
            current_directory_state = BusinessLogicUtil.get_directory_files_files_for_monitoring_mtime_without_files_to_exclude(directory_abspath=directory_abspath)  # from str to [["abspath", "mtime"]]
        ...
        # 변화 감지 ( 러프, 덜 느림)
        try:
            if BusinessLogicUtil.is_two_lists_equal(list1=current_directory_state, list2=previous_directory_state):
    ...
    def get_directory_files_files_for_monitoring_mtime_without_files_to_exclude(directory_abspath):
        files_to_exclude = [
            ...
            BusinessLogicUtil.SUCCESS_LOG,
            BusinessLogicUtil.LOCAL_PKG_CACHE_FILE,
    ...
    def get_added_files(previous_state, current_state):
        return BusinessLogicUtil.get_elements_that_list1_only_have(list1=current_state, list2=previous_state)

    @staticmethod
    def get_deleted_files(previous_state, current_state):
        return BusinessLogicUtil.get_elements_that_list1_only_have(list1=previous_state, list2=current_state)

    @staticmethod
    def get_modified_files(previous_state, current_state):
        return BusinessLogicUtil.get_different_elements(list1=current_state, list2=previous_state)
        ...
        while True:
            ...
            if os.path.isdir(directory_b):
                for target in os.listdir(directory_a):  # os.listdir 은 without walking 으로 동작한다
                    ...
                    BusinessLogicUtil.move_target_without_overwrite(target_abspath=rf'{directory_a}\{target}', dst=dst)
                ...
                for target in os.listdir(directory_b):  # os.listdir 은 without walking 으로 동작한다
                    ...
                    BusinessLogicUtil.move_target_without_overwrite(target_abspath=rf'{directory_b}\{target}', dst=dst)
            else:
            ...
            for index, directory in enumerate(directoryies_):
                if os.path.isdir(directory):
                    if FileSystemUtil.is_empty_directory(directory):
                        ...
                        BusinessLogicUtil.move_target_without_overwrite(target_abspath=directory, dst=dst)
    ...
    def explorer(file_abspath: str):
        DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
        file_abspath = str(file_abspath)
        file_abspath = file_abspath.replace('\"', '')
        file_abspath = rf'explorer "{file_abspath}"'
        DebuggingUtil.debug_as_cli(rf'{file_abspath}')
        try:
            subprocess.check_output(file_abspath, shell=True).decode('utf-8').split('\n')
        except UnicodeDecodeError:
            subprocess.check_output(file_abspath, shell=True).decode('euc-kr').split('\n')
        except subprocess.CalledProcessError:
            pass
...
class UiUtil:
    class CustomQdialog(QDialog):
        ...
        def update_text_of_clicked_btn(self, text_of_clicked_button):
            DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
            self.text_of_clicked_btn = text_of_clicked_button
        ...
        def __init__(self, ment: str, buttons=None, parent=None, is_input_text_box=False, input_text_default="", title="", auto_click_negative_btn_after_seconds: int = None, auto_click_positive_btn_after_seconds: int = None):
            ...
            self.is_input_text_box = is_input_text_box
            ...
            if self.is_input_text_box == True:
                self.box_for_editing_input_text = QLineEdit()
                self.box_for_editing_input_text.setText(self.input_text_default)
                self.box_for_editing_input_text.setFocusPolicy(Qt.StrongFocus)
                self.box_for_editing_input_text.setFocus()  # 창이 나타났을 때 focus 가 다른데 말고 입력 텍스트 박스에 있도록
            ...
            try:
                ...
                if self.buttons[0]:
                    ...
                    self.btn_positive.clicked.connect(partial(self.update_text_of_clicked_btn_and_close, self.buttons[0]))
                    ...
                    self.set_shortcut(function=partial(self.update_text_of_clicked_btn_and_close, self.buttons[0]), key_plus_key="alt+y")  # 단축키 설정
                if self.buttons[1]:
                    ...
                    self.btn_negative.clicked.connect(partial(self.update_text_of_clicked_btn_and_close, self.buttons[1]))
                    font_path = FontsUtil.GMARKETSANSTTFLIGHT_TTF
                    # font_path = FONTS.RUBIKDOODLESHADOW_REGULAR_TTF
                    self.btn_negative.setFont(Pyside6Util.get_font_for_pyside6(font_path=font_path))

                    self.set_shortcut(function=partial(self.update_text_of_clicked_btn_and_close, self.buttons[1]), key_plus_key="alt+n")  # 단축키 설정
                if self.buttons[2]:
                    self.btn_etc = QPushButton(f'{self.buttons[2]} (Alt + R)')
                    self.btn_etc.setStyleSheet("background-color: rgba(0, 0, 0, 0);  color: white;")
                    self.btn_etc.clicked.connect(partial(self.update_text_of_clicked_btn_and_close, self.buttons[2]))
                    font_path = FontsUtil.GMARKETSANSTTFLIGHT_TTF
                    # font_path = FONTS.RUBIKDOODLESHADOW_REGULAR_TTF
                    self.btn_etc.setFont(Pyside6Util.get_font_for_pyside6(font_path=font_path))

                    self.set_shortcut(function=partial(self.update_text_of_clicked_btn_and_close, self.buttons[2]), key_plus_key="alt+r")  # 단축키 설정
            except IndexError:
            ...
            if is_input_text_box:
                layout_vertical.addWidget(self.box_for_editing_input_text)
        ...
        def countdown_and_click_positive_btn(self):
            ...
            if self.seconds_remaining_until_auto_click == 0:
                if self.buttons[0]:
                    self.update_text_of_clicked_btn_and_close(self.buttons[0])
        ...
        def countdown_and_click_negative_btn(self):
            ...
            if self.seconds_remaining_until_auto_click == 0:
                if self.buttons[1]:
                    self.update_text_of_clicked_btn_and_close(self.buttons[1])
            self.seconds_remaining_until_auto_click = self.seconds_remaining_until_auto_click - 1
    ...
    class CustomDialog():
        def __init__(self, q_application: QApplication, q_wiget: QWidget, is_app_instance_mode=False, is_exec_mode=True):
            ...
            if is_exec_mode == True:
                dialog.exec()
            self.text_of_clicked_btn = dialog.text_of_clicked_btn
    ...
    class RpaProgramMainWindow(QWidget):
        ...
        def monitor_mouse_position_per_second(self):
            ...
            if self.previous_position is not None and self.previous_position == self.current_position:
                ...
                # 우측하단 꼭지점 부근 네비게이션
                if 3440 - 200 <= x <= 3440 and 1440 - 200 <= y <= 1440:
        ...
        def open_project_directory(self):
            ...
            BusinessLogicUtil.explorer(project_directory)
    ...
    class MacroWindow(QDialog):
        ...
        def __init__(self):
            ...
            #  메인창 설정
            self.setWindowTitle('.')
            self.setWindowIcon(QIcon(BusinessLogicUtil.ICON_PNG))  # 메인창 아이콘 설정
            # self.setAttribute(Qt.WA_TranslucentBackground) # 메인창 블러 설정
            # self.setWindowFlags(Qt.WindowType.FramelessWindowHint) # 메인창 최상단 프레임레스 설정
            GlobalBlur(self.winId(), hexColor=False, Acrylic=False, Dark=True, QWidget=self)
            self.setWindowFlags(Qt.WindowTitleHint | Qt.WindowCloseButtonHint)  # 최대화 최소화 버튼 숨기기
            ...
            # 매크로녹화시작 로깅
            log_title = "매크로녹화시작"
            contents = f"{BusinessLogicUtil.line_length_promised}{BusinessLogicUtil.get_time_as_('%Y-%m-%d_%H:%M:%S')}{log_title}"
        ...
        def exit_macro(self):
            ...
            BusinessLogicUtil.explorer(BusinessLogicUtil.MACRO_LOG)
        ...
        def save_macro_log(self, contents: str):
            macro_log = BusinessLogicUtil.MACRO_LOG
            BusinessLogicUtil.make_leaf_file(BusinessLogicUtil.MACRO_LOG)
    ...
    class CustomQthread(QThread):
        ...
        def run(self):
            ...
            def tmp2(q_application: QApplication):
                ...
                if dialog2.text_of_clicked_btn == "실행":
    ...
    def pop_up_as_complete(title: str, ment: str, auto_click_positive_btn_after_seconds: int, input_text_default=""):
        ...
        dialog = UiUtil.CustomQdialog(title=title, ment=ment, buttons=["확인"], is_input_text_box=is_input_text_box, input_text_default=input_text_default, auto_click_positive_btn_after_seconds=auto_click_positive_btn_after_seconds)
        dialog.exec()
        text_of_clicked_btn = dialog.text_of_clicked_btn

        if text_of_clicked_btn == "":
            DebuggingUtil.debug_as_cli(f'버튼 눌렸습니다 입니다 {text_of_clicked_btn}')
        if app == True:
                    ...
                    if type(ment) == str:
                        ...
                        try:
                            ...
                            try:
                                ...
                                playing_sounds = BusinessLogicUtil.playing_sounds
                        ...
                        BusinessLogicUtil.log_mid(log_title=f"TTS 재생시도")
                        return
                    return
            except Exception:
                DebuggingUtil.trouble_shoot("%%%FOO%%%")
...
class MathUtil:
    ...
    @staticmethod
    def get_minuites_from_(seconds):  # 큰단위로 단위변환 시 숫자는 작아지니 숫자가 작아지도록 예약된 숫자를 곱하면 된다. # seconds -> min  인 경우 큰단위로 변환되고 있고 예약된 숫자는 60 이다. 작은 숫자를 만들기 위해서 1/60 을 곱한다.
        return round(seconds / 60, 2)

    @staticmethod
    def get_minuites_and_remaining_secs(seconds):  # 큰단위로 단위변환 시 숫자는 작아지니 숫자가 작아지도록 예약된 숫자를 곱하면 된다.
        mins = seconds // 60
        secs_remaining = seconds % 60
        return mins, secs_remaining

    @staticmethod
    def get_value_splited_integer_part_and_decimal_part_from_(value):  # 큰단위로 단위변환 시 숫자는 작아지니 숫자가 작아지도록 예약된 숫자를 곱하면 된다.
        import math
...
class DbTomlUtil:
    ...
    def create_db_toml():
        try:
            ...
            if not os.path.isfile(db_abspath):
                ...
                BusinessLogicUtil.make_leaf_file(db_abspath)
    ...
    def get_db_toml_key(target_abspath):
        return BusinessLogicUtil.get_str_replaced_special_characters(target=target_abspath, replacement="_")
    ...
    def bkup_db_toml():
        try:
            DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
            BusinessLogicUtil.bkup(StateManagementUtil.DB_TOML)
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
    ...
    def delete_db_toml():
        try:
            DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
            BusinessLogicUtil.convert_as_zip_with_timestamp(StateManagementUtil.DB_TOML)
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
    ...
    def is_accesable_local_database():
        ...
        if not os.path.exists(StateManagementUtil.DB_TOML):
            BusinessLogicUtil.create_db_toml()
            return False
        else:
            return True
...
class SeleniumUtil:
    ...
    def get_driver_for_selenium():
        ...
        options = BusinessLogicUtil.get_webdriver_options_customed()
...
class BusinessLogicUtil:
    ...
    def guide_the_user_on_correct_sanitized_input(user_input: str):
        # 수정 필요.
        ...
        if TextToSpeechUtil.is_only_no(user_input):
            ...
            return "user_inputed_wrong_number"  # 나중에 enum 으로 바꾸어 보자
        return user_input
    ...
    def should_i_download_youtube_as_webm_alt():
        while True:
            dialog = UiUtil.CustomQdialog(ment="다운로드하고 싶은 URL을 제출해주세요?", buttons=["제출", "제출하지 않기"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == "제출":
                BusinessLogicUtil.download_from_youtube_to_webm_alt(dialog.box_for_editing_input_text.text())
            else:
                break
    ...
    @staticmethod
    def should_i_back_up_target():  # please_type_abspath_to_bkup
        while True:
            dialog = UiUtil.CustomQdialog(ment="빽업할 타겟경로를 입력하세요", buttons=["입력", "입력하지 않기"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == "입력":
                BusinessLogicUtil.bkup(dialog.box_for_editing_input_text.text())
            else:
                break
    ...
    def should_i_translate_eng_to_kor():
        while True:
            dialog = UiUtil.CustomQdialog(ment="WRITE SOMETHING YOU WANT TO TRANSLATE \n(FROM ENG TO KOREAN)", buttons=["Translate this", "Don't"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == "Translate this":
                BusinessLogicUtil.translate_eng_to_kor(dialog.box_for_editing_input_text.text())
            else:
                break
    ...
    def should_i_translate_kor_to_eng():
        while True:
            dialog = UiUtil.CustomQdialog(ment='번역하고 싶은 내용을 입력하세요\n(한글에서 영어로)', buttons=["번역해줘", "번역하지 않기"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == "번역해줘":
                BusinessLogicUtil.translate_kor_to_eng(dialog.box_for_editing_input_text.text())
            else:
                break
    ...
    def should_i_show_animation_information_from_web():
        while True:
            dialog = UiUtil.CustomQdialog(ment="nyaa.si 에서 검색할 내용을 입력하세요", buttons=["검색", "검색하지 않기"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == "검색":
                BusinessLogicUtil.search_animation_data_from_web(dialog.box_for_editing_input_text.text())
            else:
                break
    ...
    def should_i_find_direction_via_naver_map():
        while True:
            dialog = UiUtil.CustomQdialog(ment="어디로 길을 찾아드릴까요?", buttons=["찾아줘", "찾아주지 않아도 되"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == "찾아줘":
                BusinessLogicUtil.find_direction_via_naver_map(dialog.box_for_editing_input_text.text())
            else:
    ...
    def should_i_reboot_this_computer():
        while True:
            ...
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == MentsUtil.YES:
    ...
    def ask_something_to_ai():
        ...
        while True:
            dialog = UiUtil.CustomQdialog(ment='AI 에게 할 질문을 입력하세요', buttons=["질문하기", "질문하지 않기"], is_input_text_box=True, input_text_default=previous_question)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == "질문하기":
                BusinessLogicUtil.ask_to_web(dialog.box_for_editing_input_text.text())
            else:
                break
    ...
    def should_i_record_macro():
        while True:
            ...
            if text_of_clicked_btn == "새로 시작하기":
                if os.path.exists(BusinessLogicUtil.MACRO_LOG):
                    BusinessLogicUtil.move_target_to_trash_bin(BusinessLogicUtil.MACRO_LOG)
                BusinessLogicUtil.make_leaf_file(BusinessLogicUtil.MACRO_LOG)
                BusinessLogicUtil.explorer(BusinessLogicUtil.MACRO_LOG)
                ...
                if os.path.exists(BusinessLogicUtil.MACRO_LOG):
                    BusinessLogicUtil.make_leaf_file(BusinessLogicUtil.MACRO_LOG)
                    BusinessLogicUtil.explorer(BusinessLogicUtil.MACRO_LOG)
            elif text_of_clicked_btn == "시작하지 않기":
                break
    ...
    def should_i_do(ment: str, function, auto_click_negative_btn_after_seconds: int = 30, auto_click_positive_btn_after_seconds: int = None):
        ...
        while True:
            ...
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            print(rf'text_of_clicked_btn : {text_of_clicked_btn}')
            if text_of_clicked_btn == MentsUtil.YES:
                function()
            else:
                break
    ...
    def should_i_check_your_promised_routines_before_coding(routines: [str]):
        ...
        while True:
            if is_first_entry == True:
                TextToSpeechUtil.speak_ments(ment='혹시 코딩을 할 계획이 계신가요?, 코딩 전 루틴을 가이드할까요?', sleep_after_play=0.55)
                dialog = UiUtil.CustomQdialog(ment='혹시 코딩을 할 계획이 계신가요?, 코딩 전 루틴은 수행하셨나요?, 루틴을 가이드할까요?', buttons=["하나씩 가이드해줘", "한번에 가이드해줘", "모두 미리 수행했어"], auto_click_positive_btn_after_seconds=30)
                is_first_entry = False
            else:
                TextToSpeechUtil.speak_ments('루틴을 하나씩 가이드할까요?', sleep_after_play=0.65)
            ...
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            if text_of_clicked_btn == "모두 미리 수행했어":
                routines = []
                if len(routines) == 0:
                    TextToSpeechUtil.speak_ments("모든 루틴을 계획대로 잘 수행하셨군요, 계획대로 잘 하셨습니다, 랜덤보상을 하나드릴게요", sleep_after_play=0.65, thread_join_mode=True)
                    # 랜덤보상들 여기에 작성하기
                    BusinessLogicUtil.explorer(BusinessLogicUtil.MUSIC_FOR_WORK)
                    break
            elif text_of_clicked_btn == "하나씩 가이드해줘":
                ...
                for i in range(0, max):
                    ...
                    dialog.exec()
                    if dialog.text_of_clicked_btn == MentsUtil.DONE:
                        del routines[cursor_position]  # 리스트의 특정 인덱스 삭제
                        if len(routines) == 0:
                            TextToSpeechUtil.speak_ments("루틴을 하나하나 계획대로 잘 수행하셨군요, 계획대로 잘 하셨습니다, 미루지 않았으니 내일도 좋은일 가득할 거에요, 랜덤보상을 하나드릴게요", sleep_after_play=0.65, thread_join_mode=True)
                            # 랜덤보상들 여기에 작성하기
                            BusinessLogicUtil.explorer(BusinessLogicUtil.MUSIC_FOR_WORK)
                            break
                    elif dialog.text_of_clicked_btn == MentsUtil.I_WANT_TO_TO_DO_NEXT_TIME:
                        ...
                        dialog.exec()
                        if dialog.text_of_clicked_btn == MentsUtil.OK_I_WILL_DO_IT_NOW:
                ...
                break
                # '응, 아니 지금할게'
            elif text_of_clicked_btn == "한번에 가이드해줘":
                ...
                dialog.exec()
                if dialog.text_of_clicked_btn == MentsUtil.DONE:
                    TextToSpeechUtil.speak_ments("계획대로 잘 하셨습니다, 미루지 않았으니 내일도 좋은일 가득할 거에요, 랜덤보상을 하나드릴게요", sleep_after_play=0.65, thread_join_mode=True)
                    # 랜덤보상들 여기에 작성하기
                    BusinessLogicUtil.explorer(BusinessLogicUtil.MUSIC_FOR_WORK)
                    break
                elif dialog.text_of_clicked_btn == MentsUtil.I_WANT_TO_TO_DO_NEXT_TIME:
    ...
    def should_i_download_youtube_as_webm():
        while True:
            dialog = UiUtil.CustomQdialog(ment="다운로드하고 싶은 URL을 제출해주세요?", buttons=["제출", "제출하지 않기"], is_input_text_box=True, auto_click_negative_btn_after_seconds=30)
            dialog.exec()
            # dialog.show()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            DebuggingUtil.commentize("text_of_clicked_btn")
            DebuggingUtil.debug_as_cli(text_of_clicked_btn)
            if text_of_clicked_btn == "제출":
                BusinessLogicUtil.download_from_youtube_to_webm(dialog.box_for_editing_input_text.text())
            else:
                break
    ...
    def should_i_merge_directories():
        while True:
            dialog = UiUtil.CustomQdialog(ment="머지할 타겟경로들을 입력하세요", buttons=["입력", "입력하지 않기"], is_input_text_box=True, auto_click_negative_btn_after_seconds=10)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            directoryies = dialog.box_for_editing_input_text.text()
            if text_of_clicked_btn == "입력":
    ...
    def should_i_convert_mkv_to_wav():
        while True:
            dialog = UiUtil.CustomQdialog(ment="convert 할 MKV파일경로를 입력하세요", buttons=["입력", "입력하지 않기"], is_input_text_box=True, auto_click_negative_btn_after_seconds=10)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            file_mkv = dialog.box_for_editing_input_text.text()
            if text_of_clicked_btn == "입력":
    ...
    def should_i_gather_empty_directory():
        while True:
            dialog = UiUtil.CustomQdialog(ment="타겟경로를 순회하며 빈폴더를 예약된 폴더로 모을까요?", buttons=["모으기", "모으지 않기"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            if text_of_clicked_btn == "모으기":
                ...
                # Park4139Park4139.Tts.speak("타겟경로를 순회하며 리프 디렉토리만 예약된 폴더로 모읍니다")
                    ...
                    for dirpath, dirnames, filenames in os.walk(target_abspath, topdown=False):
                        if not dirnames and not filenames:
                            if os.path.isdir(dirpath):
                                ...
                                BusinessLogicUtil.move_target_without_overwrite(target_abspath=dirpath, dst=dst)
                TextToSpeechUtil.speak_ments("타겟경로를 순회하며 빈폴더를 예약된 폴더로 모았습니다", sleep_after_play=0.65)
            else:
                break
    ...
    def should_i_gather_useless_files():
        try:
            while True:
                dialog = UiUtil.CustomQdialog(ment="타겟경로를 순회하며 예약된 불필요한 파일을 예약된 폴더로 모을까요?", buttons=["모으기", "모으지 않기"], is_input_text_box=True)
                dialog.exec()
                text_of_clicked_btn = dialog.text_of_clicked_btn
                if text_of_clicked_btn == "모으기":
                    directory_abspath = dialog.box_for_editing_input_text.text()
                    ...
                    try:
                        ...
                        for useless_file in useless_files:
                            BusinessLogicUtil.move_target_without_overwrite(target_abspath=useless_file, dst=useless_directories)  # mkmk
                            is_target_moved_done = True
                        if is_target_moved_done == True:
                            TextToSpeechUtil.speak_ments("타겟경로를 순회하며, 약속된 불필요한 파일을, 예약된 디렉토리로 모았습니다", sleep_after_play=0.65)
                    except:
                        DebuggingUtil.trouble_shoot("%%%FOO%%%")
                    ...
                    #                         if isSpoken == False:
                    #                             TtsUtil.speak_ments("타겟경로를 순회하며 예약된 불필요한 파일을 예약된 폴더로 모았습니다", sleep_after_play=0.65)
                    #                             isSpoken = True
                    # except:
                    #     DebuggingUtil.trouble_shoot("%%%FOO%%%")
                elif text_of_clicked_btn == "모으지 않기":
                    break
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
    ...
    def should_i_gather_special_files():
        while True:
            dialog = UiUtil.CustomQdialog(ment="타겟경로를 순회하며 특별한 파일을 예약된 폴더로 모을까요?", buttons=["모으기", "모으지 않기"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            if text_of_clicked_btn == "모으기":
                target_abspath = dialog.box_for_editing_input_text.text()
                        ...
                        for file in files:
                            ...
                            for special_file in special_files:
                                if special_file in os.path.basename(file_path):
                                    ...
                                    BusinessLogicUtil.move_target_without_overwrite(target_abspath=file_path, dst=dst)
                                    if isSpoken == False:
                                        TextToSpeechUtil.speak_ments("타겟경로를 순회하며 특별한 파일을 예약된 폴더로 모았습니다", sleep_after_play=0.65)
                                        isSpoken = True
            else:
                break
    ...
    def should_i_classify_special_files():
        while True:
            dialog = UiUtil.CustomQdialog(ment="타겟경로를 순회하며 파일들을 예약된 키워드로 된 폴더로 분류할까요?", buttons=["분류하기", "분류하지 않기"], is_input_text_box=True)
            dialog.exec()
            text_of_clicked_btn = dialog.text_of_clicked_btn
            if text_of_clicked_btn == "분류하기":
                target_abspath = dialog.box_for_editing_input_text.text()
                target_abspath = target_abspath.strip()
                target_abspath = target_abspath.replace("\"", "")
                target_abspath = target_abspath.replace("\'", "")
                ...
                if os.path.isdir(target_abspath):
                    for root, dirs, files in os.walk(target_abspath, topdown=False):  # os.walk()는 with walking 으로 동작한다
                        for file in files:
                            ...
                            for special_file in special_files:
                                if special_file in os.path.basename(file_path):
                                    print(rf"target file to be classifed :{file_path}")
                                    FileSystemUtil.make_leaf_directory(rf"{dst}\{special_file}")
                                    BusinessLogicUtil.move_target_without_overwrite(target_abspath=file_path, dst=rf"{dst}\{special_file}")
                                    if isSpoken == False:
                                        TextToSpeechUtil.speak_ments("타겟경로를 순회하며 특별한 파일을 예약된 폴더로 분류했습니다", sleep_after_play=0.65)
                                        isSpoken = True
            else:
                break
    ...
    def make_me_go_to_sleep():
        DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
        BusinessLogicUtil.count_of_make_me_go_to_sleep.append("!!")
        TextToSpeechUtil.speak_ments(ment='너무 늦은 시간입니다', sleep_after_play=0.95, thread_join_mode=True)
        ...
        if random.random() >= 0.5:
            ...
            if random.random() >= 0.5:
                ...
                TextToSpeechUtil.speak_ments(ment='늦게까지 개발하고 내일 너무 피곤하지 않을까요', sleep_after_play=0.95, thread_join_mode=True)
                if random.random() >= 0.5:
                    TextToSpeechUtil.speak_ments(ment='내일 하루를 망쳐버릴 수도 있어요', sleep_after_play=0.95, thread_join_mode=True)
        if random.random() >= 0.5:
            TextToSpeechUtil.speak_ments(ment='새벽 입니다', sleep_after_play=0.95, thread_join_mode=True)
            TextToSpeechUtil.speak_ments(ment='이제 그만 주무세요 정말로', sleep_after_play=0.95, thread_join_mode=True)
        if len(BusinessLogicUtil.count_of_make_me_go_to_sleep) == 10:
            TextToSpeechUtil.speak_ments(ment='아무래도 안되겠군요', sleep_after_play=0.95, thread_join_mode=True)
            TextToSpeechUtil.speak_ments(ment='이제 하나씩 종료를 할겁니다', sleep_after_play=0.95, thread_join_mode=True)
            TextToSpeechUtil.speak_ments(ment='팟플레이어를 종료합니다', sleep_after_play=0.95, thread_join_mode=True)
            BusinessLogicUtil.should_i_do(ment="알송을 종료합니다", function=partial(BusinessLogicUtil.taskkill, 'ALSong.exe'), auto_click_positive_btn_after_seconds=20)
                            ...
                            if BusinessLogicUtil.is_regex_in_contents(line, regex):
                                ...
                                if 60 < diff.seconds:
                                    try:
                                        ...
                                        break
                os.chdir(StateManagementUtil.PROJECT_DIRECTORY)
                UiUtil.pop_up_as_complete(title="작업완료메시지", ment=f"예약된 백업이 완료되었습니다\n{target_abspath}", auto_click_positive_btn_after_seconds=5)
                break
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
            os.chdir(StateManagementUtil.PROJECT_DIRECTORY)
    ...
    def monitor_target_edited_and_bkup(target_abspath: str):
        ...
        try:
            if os.path.isfile(target_abspath):
                if BusinessLogicUtil.is_file_changed(target_abspath):
                    BusinessLogicUtil.bkup(target_abspath)
            elif os.path.isdir(target_abspath):
                if BusinessLogicUtil.is_directory_changed(target_abspath):
                    BusinessLogicUtil.bkup(target_abspath)
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
    ...
    def monitor_target_edited_and_sync(target_abspath: str):
        ...
        try:
            if os.path.isfile(target_abspath):
                if BusinessLogicUtil.is_file_changed(target_abspath):
                    BusinessLogicUtil.sync_directory_local(target_abspath)
            elif os.path.isdir(target_abspath):
                BusinessLogicUtil.sync_directory_local(target_abspath)
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
    ...
    def download_clip(url: str):
        ...
        while True:
            ...
            try:
                ...
                if src != os.getcwd():  # 여기 또 os.getcwd() 있는 부분 수정하자..
                    BusinessLogicUtil.move_target_without_overwrite(src, src_renamed)
            ...
            try:
                ...
                UiUtil.pop_up_as_complete(title="작업완료메시지", ment=f"{src_moved} 가 다운로드 되었습니다", auto_click_positive_btn_after_seconds=5)
                # 다운로드된 영상을 재생할까요?
                # Park4139Park4139.Tts.speak(ment = "다운로드된 영상을 재생합니다")
    ...
    def download_clip_alt(url: str):
        ...
        while True:
            ...
            if video_id == "" or audio_id == "" == 1:
                dialog = UiUtil.CustomQdialog(ment=f"에러코드[E000]\n불완전한 다운로드 명령어가 감지되었습니다. 에러코드를 관리자에게 문의해주세요", buttons=["확인"], is_input_text_box=True, input_text_default=url)
                DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
                dialog.exec()
                DebuggingUtil.commentize("불완전한 다운로드 명령어가 감지되었습니다....")
                TextToSpeechUtil.speak_ments(ment="불완전한 다운로드 명령어가 감지되었습니다", sleep_after_play=0.65)
    ...
    def download_from_youtube_to_webm(urls_from_prompt):
        ...
        while True:
            ...
            if len(urls) == 0:
                UiUtil.pop_up_as_complete(title="작업완료메시지", ment=f"다운로드할 대상이 없습니다", auto_click_positive_btn_after_seconds=5)
            ...
            for url in urls:
                ...
                if '&list=' in url:
                    ...
                    TextToSpeechUtil.speak_ments(ment="여러개의 다운로드 목록이 확인되었습니다", sleep_after_play=0.65)
    ...
    def move_target_without_overwrite(target_abspath, dst):
        ...
        try:
            ...
            src_dirname = os.path.dirname(target_abspath)
            src_basename = os.path.basename(target_abspath)
            ...
            new_named = None
            print(rf'src_dirname : {src_dirname}')
            print(rf'src_basename : {src_basename}')
            print(rf'src_n : {src_n}')
            print(rf'src_x : {src_x}')
            print(rf'spaceless_time_pattern : {spaceless_time_pattern}')
            print(rf'new_named : {new_named}')
            if os.path.isfile(target_abspath):
                if os.path.exists(rf"{dst}\{os.path.basename(target_abspath)}"):
                    ment = f"이미 중복된 파일명이 미래경로에 있습니다. 타임스탬프를 넣어 타겟이동을 시도 합니다\n{target_abspath}"
                    dialog = UiUtil.CustomQdialog(ment=ment, buttons=["확인"], auto_click_positive_btn_after_seconds=1)
                    dialog.exec()
                    new_named = rf'{src_dirname}\{src_n}{spaceless_time_pattern}{src_x}'
                    print(rf'target_abspath : {target_abspath}')
                    print(rf'new_named : {new_named}')
                    os.rename(target_abspath, new_named)
                else:
                    new_named = target_abspath
                ...
                try:
                    ...
                    DebuggingUtil.commentize('타겟 이동 샐패, xcopy 로 이동 재시도 시작')
                    future_target_abspath = rf"{dst}\{src_basename}"
                    if os.path.exists(future_target_abspath):
                        ment = f"이미 중복된 파일명이 미래경로에 있습니다. 타임스탬프를 넣어 타겟이동을 시도 합니다\n{target_abspath}"
                        dialog = UiUtil.CustomQdialog(ment=ment, buttons=["확인"], auto_click_positive_btn_after_seconds=1)
                        dialog.exec()
                        future_target_abspath = rf'{src_dirname}\{src_n}_{spaceless_time_pattern}{src_x}'
                    print(rf'target_abspath : {target_abspath}')
                    print(rf'future_target_abspath : {future_target_abspath}')
                    try:
                        BusinessLogicUtil.xcopy_without_overwrite(target_abspath=target_abspath, future_target_abspath=future_target_abspath)
                        if os.path.exists(future_target_abspath):
                            ...
                            BusinessLogicUtil.should_i_do(ment=rf"이 파일들을 쓰레기통으로 이동할까요?{target_abspath}", function=partial(BusinessLogicUtil.move_target_to_trash_bin, target_abspath), auto_click_negative_btn_after_seconds=30)
                    except:
                        DebuggingUtil.commentize('타겟 이동 샐패,  xcopy 로도 이동 실패')
            elif os.path.isdir(target_abspath):
                if os.path.exists(rf"{dst}\{os.path.basename(target_abspath)}"):
                    ment = f"이미 중복된 디렉토리명이 미래경로에 있습니다. 타임스탬프를 넣어 타겟이동을 시도 합니다\n{target_abspath}"
                    dialog = UiUtil.CustomQdialog(ment=ment, buttons=["확인"], auto_click_positive_btn_after_seconds=1)
                    dialog.exec()
                ...
                shutil.move(target_abspath, new_named)
        except:
    ...
    def move_without_overwrite_via_robocopy(target_abspath, dst):  # 명령어 자체가 안되는데 /mir 은 되는데 /move 안된다
        ...
        try:
            ...
            if os.path.exists(rf'{dst}\{os.path.dirname(target_abspath)}'):
                BusinessLogicUtil.move_target_to_trash_bin(target_abspath)
    ...
    def is_file_edited(target_abspath: str):
        ...
        try:
            ...
            db = BusinessLogicUtil.read_db_toml()
        ...
        while True:
            try:
                ...
                if lines == subprocess.CalledProcessError:
                    break
                if "" in lines:
                    DebuggingUtil.debug_as_cli(f"{inspect.currentframe().f_code.co_name} - add success")
                    ...
                    if "nothing to commit, working tree clean" in lines:
                        DebuggingUtil.debug_as_cli(f"{inspect.currentframe().f_code.co_name} - commit success")
                        ...
                        if "Everything up-to-date" or "branch 'main' set up to track 'origin/main'." in lines:
                            ...
                            DebuggingUtil.debug_as_cli(f"{inspect.currentframe().f_code.co_name} - push success")
                            ...
                            DebuggingUtil.debug_as_cli(f"{inspect.currentframe().f_code.co_name} - push fail")
                    else:
                        DebuggingUtil.debug_as_cli(f"{inspect.currentframe().f_code.co_name} - commit fail")
                else:
                    DebuggingUtil.debug_as_cli(f"{inspect.currentframe().f_code.co_name} - add fail")
            except:
    ...
    def save_all_list():  # 수정 필요, 모든 파일 디렉토리 말고, 특정디렉토리로 수정한는 것도 할것
        ...
        DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
        DebuggingUtil.debug_as_cli("____________________________________________________________________________________________________________ directory $cache_all_tree/all_tree.txt/proper_tree.txt 생성")
        os.system('chcp 65001 >nul')
        ...
        DebuggingUtil.debug_as_cli("____________________________________________________________________________________________________________ all_tree.txt/proper_tree.txt 내용 지우기")
    ...
    def search_animation_data_from_web(text_of_clicked_btn):
        ...
        try:
            while 1:
                # answer = "tate no"
                DebuggingUtil.commentize(f"{text_of_clicked_btn} 입력되었습니다")
                DebuggingUtil.debug_as_cli(text_of_clicked_btn)
                special_prefixes = " ".join(["subsplease", "1080"]) + " "
                query = urllib.parse.quote(f"{special_prefixes}{text_of_clicked_btn}")
                if query == "":
                ...
                for i in results:
                    if type(i) is str:
                        if BusinessLogicUtil.is_regex_in_contents_with_case_ignored(contents=str(i).strip(), regex=text_of_clicked_btn):
                ...
                if len(magnets) == len(titles):
                    ...
                    text_of_clicked_btn = dialog.text_of_clicked_btn
                    DebuggingUtil.debug_as_cli(text_of_clicked_btn)
                    if text_of_clicked_btn == "계속하지 않기":
                ...
                while True:
                    ...
                    TextToSpeechUtil.speak_ments(ment, sleep_after_play=0.65)
                    dialog = UiUtil.CustomQdialog(ment=ment, buttons=["계속하기", "그만하기", "크롤링 결과 다시보기"], is_input_text_box=True)
                    dialog.exec()
                    text_of_clicked_btn = dialog.text_of_clicked_btn
                    text_of_from_input_box = str(dialog.box_for_editing_input_text.text()).strip()
                    DebuggingUtil.debug_as_cli(f"text_of_box_for_editing_input_text : {text_of_from_input_box}")
                    DebuggingUtil.debug_as_cli(f"text_of_clicked_btn : {text_of_clicked_btn}")
                    if text_of_clicked_btn == "그만하기":
                        TextToSpeechUtil.speak_ments(ment="네 그만하겠습니다", sleep_after_play=0.65)
                        break
                    elif text_of_clicked_btn == "크롤링 결과 다시보기":
                        dialog = UiUtil.CustomQdialog(ment=results_about_titles_and_magnets, buttons=["확인"])
                        dialog.exec()
                    elif text_of_clicked_btn == "계속하기":
    ...
    def debug_as_gui(context: str, is_app_instance_mode=False, input_text_default=""):
        ...
        if input_text_default == "":
            ...
            is_input_text_box = True
        dialog = UiUtil.CustomQdialog(ment=f"{context}", buttons=["확인"], is_input_text_box=is_input_text_box, input_text_default=input_text_default)
        DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
        dialog.exec()
        text_of_clicked_btn = dialog.text_of_clicked_btn
        if text_of_clicked_btn == "":
            DebuggingUtil.debug_as_cli(f'누르신 버튼은 {text_of_clicked_btn} 입니다')
        if is_app_instance_mode == True:
            if isinstance(app_foo, QApplication):
                app_foo.exec()
        if is_app_instance_mode == True:
    ...
    def translate_eng_to_kor_via_googletrans(text: str):  # 수정할것 update 되었다는데 googletrans 업데이트 해보자
        ...
        # naver papago api 서비스는 2024 년 종료가 된다...ㅜ
    ...
    def get_font_name_for_mataplot(font_abspath):
        ...
        if not os.path.exists(future_abspath):
            BusinessLogicUtil.xcopy_with_overwrite(target_abspath=font_abspath, future_target_abspath=future_abspath)
        font_name = fm.FontProperties(fname=future_abspath).get_name()
        return font_name

    @staticmethod
    def speak_after_x_min(mins: int):
        DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
        TextToSpeechUtil.speak_ments(f"{str(mins)}분 경과했습니다", sleep_after_play=0.65)
    ...
    def get_comprehensive_weather_information_from_web():
        ...
        try:
            while 1:
                ...
                # 함수가 break 로 끝이 나면 창들이 사라지는 코드에서 창을 닫아야 dialog 들이 사라지도록 dialog 를 global 처리를 해두었음.
                global dialog4
                global dialog3
                global dialog2
                global dialog1
    ...
    def bkup_biggest_targets():
        ...
        for biggest_target in BusinessLogicUtil.biggest_targets:
            BusinessLogicUtil.bkup(f'{biggest_target}')

    @staticmethod
    def bkup_smallest_targets():
        DebuggingUtil.commentize(f"__________________________________________________{inspect.currentframe().f_code.co_name}")
        DebuggingUtil.commentize(f"smallest_targets에 대한 빽업을 시도합니다")
        for target in BusinessLogicUtil.smallest_targets:
            BusinessLogicUtil.bkup(f'{target}')
    ...
    def classify_targets_between_smallest_targets_biggest_targets():
        ...
        for target in targets:
            ...
            if target_size_megabite <= 300:
                BusinessLogicUtil.smallest_targets.append(target.strip())

            elif 300 < target_size_megabite:
                BusinessLogicUtil.biggest_targets.append(target.strip())
            else:
        ...
        smallest_targets = [i for i in targets if i not in BusinessLogicUtil.biggest_targets]
        for target in BusinessLogicUtil.smallest_targets:
            print(target)

        DebuggingUtil.commentize('biggest_target 출력')
        for target in BusinessLogicUtil.biggest_targets:
            print(target)
        pass
        ...
        if not storages:
            ...
            for storage in storages:
                ...
                try:
                    ...
                    BusinessLogicUtil.move_target_without_overwrite(storage, dst)
                except FileNotFoundError:
    ...
    def run_targets_promised():
        ...
        try:
            targets = [
                ...
                BusinessLogicUtil.PYCHARM64_EXE,
                StateManagementUtil.PROJECT_DIRECTORY,
                BusinessLogicUtil.SERVICE_DIRECTORY,
            ]
            for target in targets:
                BusinessLogicUtil.explorer(target)
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
    ...
    def run_up_and_down_game():
        ...
        TextToSpeechUtil.speak_ments(ment, sleep_after_play=0.65)
        dialog = UiUtil.CustomQdialog(ment=ment, buttons=["start", "exit"], is_input_text_box=False)
        dialog.exec()
        user_input = None
        ...
        if text_of_clicked_btn == "start":
            ...
            while left_oportunity >= 0:
                if left_oportunity == 0:
                    ...
                    dialog.exec()
                    break
                elif is_game_strated == False:
                    ment = f"type number between 1 to 100."
                    TextToSpeechUtil.speak_ments(ment, sleep_after_play=0.65)
                    dialog = UiUtil.CustomQdialog(ment=ment, buttons=["submit"], is_input_text_box=True)
                    dialog.exec()
                    user_input = DataStructureUtil.guide_the_user_on_correct_sanitized_input(dialog.box_for_editing_input_text.text())
                    if not user_input == "user_inputed_wrong_number":
    ...
    def run_console_blurred_core_as_scheduler(q_application: QApplication):
        ...
        while True:
            ...
            if scheduler_loop_cnt == 0:
                ...
                BusinessLogicUtil.should_i_check_your_promised_routines_before_coding(BusinessLogicUtil.routines_promised)

                BusinessLogicUtil.should_i_do(ment="예약된 타겟들을 실행할까요?", function=BusinessLogicUtil.run_targets_promised, auto_click_negative_btn_after_seconds=20)

                BusinessLogicUtil.should_i_do(ment="예약된 불필요한 프로그램을 종료할까요?", function=BusinessLogicUtil.taskkill_useless_programs, auto_click_negative_btn_after_seconds=20)

                # TtsUtil.speak_with_queue("랜덤스케쥴을 실행했습니다") # 루프 돌때마다 1번만 speak_queue 에 쌓인 내용을 speak_as_sync 읇어주도록
                BusinessLogicUtil.should_i_do(ment="랜덤 스케쥴을 실행할까요?", function=BusinessLogicUtil.do_random_schedules, auto_click_negative_btn_after_seconds=20)

                if not BusinessLogicUtil.is_accesable_local_database():
                    TextToSpeechUtil.speak_ments("로컬 데이터베이스에 접근할 수 없어 접근이 가능하도록 설정했습니다", sleep_after_play=0.65)

                BusinessLogicUtil.classify_targets_between_smallest_targets_biggest_targets()
            ...
            if 0 <= int(HH) <= 24 and int(ss) == 0:
                BusinessLogicUtil.monitor_target_edited_and_bkup(target_abspath=BusinessLogicUtil.PARK4139_ARCHIVE_TOML)  # seconds_performance_test_results : ['11.95sec', '26.78sec', '11.94sec', '3.7sec', '11.72sec']
                ...
                if int(HH) == 11 and int(mm) == 30:
                    ...
                    TextToSpeechUtil.speak_ments('용량이 큰 예약된 타겟들을 빽업을 수행 시도합니다', sleep_after_play=0.65, thread_join_mode=True)
                    BusinessLogicUtil.bkup_biggest_targets()
                    TextToSpeechUtil.speak_ments('용량이 작은 예약된 타겟들을 빽업을 수행 시도합니다', sleep_after_play=0.65, thread_join_mode=True)
                    BusinessLogicUtil.bkup_smallest_targets()
                ...
                if int(mm) % 1 == 0:
                    BusinessLogicUtil.should_i_do(ment="1분 간격 테스트", function=BusinessLogicUtil.speak_server_hh_mm, auto_click_negative_btn_after_seconds=3)
                if int(mm) % 15 == 0:
                ...
                if int(mm) % 60 == 0:
                    ...
                    BusinessLogicUtil.should_i_do(ment="쓰레기통을 비울까요?", function=BusinessLogicUtil.empty_recycle_bin, auto_click_negative_btn_after_seconds=10)
                    BusinessLogicUtil.should_i_do(ment="오늘 시간정보를 말씀드릴까요?", function=BusinessLogicUtil.speak_today_time_info, auto_click_negative_btn_after_seconds=30)
                    BusinessLogicUtil.monitor_target_edited_and_bkup(target_abspath=StateManagementUtil.PROJECT_DIRECTORY)
            ...
            if yyyy != yyyy_previous_loop:
                ment = f'{yyyy} 신년입니다! 새해에는 반드시 좋은일이 가득하실겁니다!!'
                TextToSpeechUtil.speak_ments(ment=ment, sleep_after_play=0.65, thread_join_mode=True)
                BusinessLogicUtil.should_i_do(ment=ment, function=BusinessLogicUtil.empty_recycle_bin, auto_click_positive_btn_after_seconds=100)
    ...
    def run_console_blurred_as_scheduler():
        ...
        if dialog.text_of_clicked_btn == "실행":
            print(StateManagementUtil.PROJECT_DIRECTORY)
            os.chdir(StateManagementUtil.PROJECT_DIRECTORY)
        if dialog.text_of_clicked_btn == "실행하지 않기":
            sys.exit()
