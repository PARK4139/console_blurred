            ...
            if text_of_clicked_btn == "모두 미리 수행했어":
                ...
                for i in range(0, max):
                    ...
                    if dialog.text_of_clicked_btn == MentsUtil.DONE:
                        ...
                        if len(routines) == 0:
                            ment ="루틴을 하나하나 계획대로 잘 수행하셨군요, 계획대로 잘 하셨습니다, 미루지 않았으니 내일도 좋은일 가득할 거에요, 랜덤보상을 하나드릴게요"
                            TextToSpeechUtil.speak_ments(ment, sleep_after_play=0.65)
                            UiUtil.pop_up_as_complete(title="작업완료보고", ment=ment, auto_click_positive_btn_after_seconds=5)
                            # 랜덤보상들 여기에 작성하기
                            FileSystemUtil.explorer(StateManagementUtil.MUSIC_FOR_WORK)
                            ...
                            if BusinessLogicUtil.is_regex_in_contents(line, regex):
                                ...
                                if 60 < diff.seconds:
                                    try:
                                        ...
                                        break
                os.chdir(StateManagementUtil.PROJECT_DIRECTORY)
                UiUtil.pop_up_as_complete(title="작업완료보고", ment=f"예약된 백업이 완료되었습니다\n{target_abspath}", auto_click_positive_btn_after_seconds=5)
                break
        except:
            DebuggingUtil.trouble_shoot("%%%FOO%%%")
            os.chdir(StateManagementUtil.PROJECT_DIRECTORY)
                    ...
                    if "nothing to commit, working tree clean" in lines:
                        ...
                        if "Everything up-to-date" or "branch 'main' set up to track 'origin/main'." in lines:
                            ...
                            if int('08') <= int(BusinessLogicUtil.get_time_as_('%H')) <= int('23'):  # 자는데 하도 시끄러워서 추가한 코드
                                ...
                                BusinessLogicUtil.should_i_do(ment="깃허브에서 직접확인하시겠요요?", function=partial(FileSystemUtil.explorer, GIT_HUB_REPOSITORY_URL), auto_click_negative_btn_after_seconds=20)
                                break
                            break
                        else:
        ...
        if text_of_clicked_btn == "start":
            ...
            while left_oportunity >= 0:
                if left_oportunity == 0:
                    ...
                    dialog.exec()
                    user_input = BusinessLogicUtil.guide_the_user_on_correct_sanitized_input(dialog.box_for_editing_input_text.text())
    ...
    def run_console_blurred_core_as_scheduler(q_application: QApplication):
        ...
        while True:
            ...
            if scheduler_loop_cnt == 0:
                ...
                BusinessLogicUtil.should_i_do(ment="백업할 타겟들을 크기에 따라 분류를 해둘까요?", function=BusinessLogicUtil.classify_targets_between_smallest_targets_biggest_targets, auto_click_negative_btn_after_seconds=20)
            ...
            if 0 <= int(HH) <= 24 and int(ss) == 0:
                BusinessLogicUtil.monitor_target_edited_and_bkup(target_abspath=StateManagementUtil.PARK4139_ARCHIVE_TOML)  # seconds_performance_test_results : ['11.95sec', '26.78sec', '11.94sec', '3.7sec', '11.72sec']
                ...
                if int(mm) % 60 == 0:
                    ...
                    BusinessLogicUtil.should_i_do(ment="쓰레기통을 비울까요?", function=BusinessLogicUtil.empty_recycle_bin, auto_click_negative_btn_after_seconds=10)
                    BusinessLogicUtil.should_i_do(ment="오늘 시간정보를 말씀드릴까요?", function=TextToSpeechUtil.speak_today_time_info, auto_click_negative_btn_after_seconds=30)
            ...
            if BusinessLogicUtil.is_two_time_same(time1=server_time, time2=datetime(year=2024, month=1, day=2, hour=14, minute=51, second=0)):
                BusinessLogicUtil.speak_that_service_is_in_preparing()
