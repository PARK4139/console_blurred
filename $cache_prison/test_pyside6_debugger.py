...
class debugger:
    def __init__(self):
        DebuggingUtil.commentize('print(self.__name__+" is initialized")')
        print(self.__name__+" is initialized")
        DebuggingUtil.commentize('super().__init__()')
        super().__init__()


    def print_built_in_info(thing_curious):
        DebuggingUtil.commentize(f"{inspect.currentframe().f_code.co_name} {str(thing_curious.__code__.co_varnames)}")
        print("_______________________________________________________________ " + str() + "("+str(thing_curious)+") s")
        DebuggingUtil.commentize("print(inspect.getsource(thing_curious))")
        print(inspect.getsource(thing_curious))
        DebuggingUtil.commentize("for i in inspect.getmembers(thing_curious_):")
        for i in inspect.getmembers(thing_curious):
            print(i)
        DebuggingUtil.commentize("print(help(thing_curious))")
        print(help(thing_curious))
        DebuggingUtil.commentize("[x for x in dir(thing_curious) if '__' not in x]")
    ...
    def print_function_info(thing_curious):
        DebuggingUtil.commentize(f"{inspect.currentframe().f_code.co_name} {str(thing_curious.__code__.co_varnames)}")
        print(help(thing_curious))
        StateManagementUtil.debug_as_cli(f'# of the Arguments : {thing_curious.__code__.co_argcount}')
