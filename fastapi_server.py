...
@app.get("/api/{specific_objects_name}")
async def get_specific_object_json_db(specific_objects_name):
    DebuggingUtil.print_via_colorama(f"{inspect.currentframe().f_code.co_name}() 호출되었습니다", colorama_color=ColoramaColorUtil.LIGHTWHITE_EX)
    with open(StateManagementUtil.DB_JSON, encoding="utf-8") as file:
        data = json.load(file)
        if data.get(specific_objects_name):
            for object in data.get(specific_objects_name):
                print(rf'object["id"] : {object["id"]} object["title"] : {object["title"].replace("\n", "")} object["href"] : {object["href"].replace("\n", "")} object["description"] : {object["description"].replace("\n", "")}')
        else:
            DebuggingUtil.print_as_fail(rf"'{specific_objects_name}'에 해당하는 객체를 찾을 수 없습니다.")
    with open(StateManagementUtil.DB_JSON, encoding="utf-8") as file:
        try:
            object = json.load(file)[specific_objects_name]
            object = JSONResponse(object)
            # DebuggingUtil.print_via_colorama(rf'json.dumps(object_specific.body) : {json.dumps(object_specific.body)}', colorama_color=ColoramaColorUtil.LIGHTBLACK_EX)
        except KeyError:
            DebuggingUtil.print_as_fail(rf'{os.path.basename(StateManagementUtil.DB_JSON)} 에서 {specific_objects_name}는 존재하지 않습니다')
            return {"info": rf"{os.path.basename(StateManagementUtil.DB_JSON)} 에서 해당 객체는 존재하지 않습니다"}
    return object
...
# TodoItem restful api 설계 sample
...
def read_item(item_id: int, q: Union[str, None] = None):
    return {"item_id": item_id, "q": q}  # 이걸 활용하면 다른 페이지로 데이터 전달을 할수 있겠지
