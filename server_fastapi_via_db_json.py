...
from pkg_park4139 import StateManagementUtil, FastapiUtil, FileSystemUtil, DebuggingUtil, FastapiUtil, BusinessLogicUtil
...
MEMBERS = FastapiUtil.init_and_update_json_file(MEMBERS_JSON)



@app.on_event("startup") # deprecated 되었음 lifespan 스타일로 대체해야함.
async def startup_event():
    ...
    DebuggingUtil.print_ment_blue(f"더미를 추가로 {dummy_cnt} 개 생성하였습니다 총 {len(dummy_data)}개의 데이터가 {os.path.basename(MEMBERS_JSON)}에 저장되어 있습니다")
...
@app.post("/member")
async def create_member(member: FastapiUtil.Member):
    member.id = uuid4().hex + BusinessLogicUtil.get_time_as_('%Y%m%d%H%M%S%f') + CipherUtil.get_random_alphabet()
    ...
    DebuggingUtil.print_ment_green(f"member 이 성공적으로 생성되었습니다.")
...
@app.put("/member")
async def update_member(id: str, member: FastapiUtil.Member):
    for i, member_ in enumerate(MEMBERS):
        if member_["id"] == id:
            MEMBERS[i] = member.model_dump() # 위에서 코드는 id 도 재초기화
            # MEMBERS[i] = membe
            MEMBERS[i]["id"] = id  # 기존 id 로 초기화
            with open(MEMBERS_JSON, "w", encoding="utf-8") as file:
                json.dump(MEMBERS, file, indent=4, ensure_ascii=False)
            DebuggingUtil.print_ment_light_white(rf'member_ : {member_}')
            DebuggingUtil.print_ment_light_yellow(f"아이디 {id} 인 member 이 성공적으로 업데이트되었습니다")
...
async def delete_member_by_id(id: str):
    for i, member in enumerate(MEMBERS):
        ...
        if member["id"] == id:
            ...
            DebuggingUtil.print_ment_red(f"아이디 {id} 인 member 이 성공적으로 삭제되었습니다.")
...
async def random_member():
    if len(MEMBERS) > 0:
        ...
        DebuggingUtil.print_ment_blue(f"{MEMBERS_JSON}에서 랜덤으로 member를 가져왔습니다")
        return member_choiced
    else:
        DebuggingUtil.print_ment_blue(rf"{MEMBERS_JSON}에 members가 없습니다")
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        host=UvicornUtil.Settings.host,  # class 를 사용하면 tuple 로 오며, str(tuple) 이렇게 사용할 수 없고, tuple[0] 으로 가져와야 하네. js 의 destructon 문법처럼 py의 unpacking 을 사용하는 방법이 있으나 변수 새로 생성해야함.
