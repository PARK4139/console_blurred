...
class StateManagementUtil:
    ...
    LOCAL_PKG_CACHE_FILE = rf'{PROJECT_DIRECTORY}\pkg_park4139_for_windows\__pycache__\__init__.cpython-312.pyc'
    ...
    NAV_ITEMS_JSON = rf"{PROJECT_DIRECTORY}\$cache_json\nav_items.json"
    PYCHARM64_EXE: str
...
class FastapiUtil:
    # def import_pkg_park4139_for_windows():  # 명령어 자체가 안되는데 /mir 은 되는데 /move 안된다
    # try:
    # 파이썬 프로젝트 외부 패키지 import 시키는 방법 : 일반적으로는 불가능하다.
    # services_directory_abspath = os.path.dirname(os.path.dirname(__file__))
    # external_pkg_abspath = rf'{services_directory_abspath}\archive_py\pkg_park4139_for_windows'
    # print(rf'external_pkg_abspath : {external_pkg_abspath}')
    # os.environ['pkg_park4139_for_windows_ABSPATH'] = rf'{external_pkg_abspath}'
    # sys.path.append(external_pkg_abspath)
    # print(rf'sys.path : {sys.path}')
    # from ..archive_py.pkg_park4139_for_windows import FileSystemUtil, StateManagementUtil, TestUtil
    # except:
    #     pass
    # try:
    # 대안 global pkg 로 복사하고 쓰고 실행후에는 프로젝트 내에서 패키지를 삭제한다 > 자동화할것
    # 의존성을 추가하기 위해서, pkg_park4139_for_windows의 .venv 도 따라 복사한다.
    # CURRENT_PROJECT_DIRECTORY= os.path.dirname(__file__)
    # SERVICES_DIRECTORY = os.path.dirname(CURRENT_PROJECT_DIRECTORY)
    # EXTERNAL_PKG_ABSPATH = rf'{SERVICES_DIRECTORY}\archive_py\pkg_park4139_for_windows'
    # # VIRTUAL_PKG_ABSPATH = rf'{CURRENT_PROJECT_DIRECTORY}\.venv\Lib\site-packages'
    # INTERNAL_PKG_ABSPATH = rf'{CURRENT_PROJECT_DIRECTORY}\pkg_park4139_for_windows'
    ...
    class Board(BaseModel):
        id: int = Field(default_factory=lambda: FastapiUtil.fill_increment_id_auto(), alias="_id")  # 수정대기
        title: str
        content: str
...
class BusinessLogicUtil:
    ...
    def update_global_pkg_park4139_for_windows():
        DebuggingUtil.commentize(f"{inspect.currentframe().f_code.co_name}()")
        local_pkg = rf"{StateManagementUtil.PROJECT_DIRECTORY}\pkg_park4139_for_windows"
        global_pkg = rf"C:\Python312\Lib\site-packages\pkg_park4139_for_windows"
        updateignore_txt = rf"{StateManagementUtil.PROJECT_DIRECTORY}\pkg_park4139_for_windows\updateignore.txt"
        try:
